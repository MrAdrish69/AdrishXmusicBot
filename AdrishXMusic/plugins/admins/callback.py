...
from AdrishXMusic import YouTube, app
from AdrishXMusic.core.call import Adrish
from AdrishXMusic.misc import SUDOERS, db
from AdrishXMusic.utils.database import (
    is_active_chat,
    is_music_playing,
    is_muted,
    is_nonadmin_chat,
...
from AdrishXMusic.utils.decorators.language import languageCB
from AdrishXMusic.utils.formatters import seconds_to_min
from AdrishXMusic.utils.inline.play import panel_markup_1, stream_markup, telegram_markup
from AdrishXMusic.utils.stream.autoclear import auto_clean
from AdrishXMusic.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Adrish.unmute_stream(chat_id)
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await Adrish.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await Adrish.skip_stream(chat_id, link, video=status)
            except Exception:
            ...
            try:
                await Adrish.skip_stream(chat_id, file_path, video=status)
            except Exception:
                return await mystic.edit_text(_["call_9"])
            ...
            try:
                await Adrish.skip_stream(chat_id, videoid, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(_["call_9"])
            button = telegram_markup(_, chat_id)
            run = await CallbackQuery.message.reply_photo(
            ...
            try:
                await Adrish.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(_["call_9"])
            if videoid == "telegram":
                button = telegram_markup(_, chat_id)
